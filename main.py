...
import aiofiles
import aiohttp
from Buy import solana_client, execute_buy_operation, load_pools_data
...
def load_pools_data(file_path='pools.json'):
    """Load pools data from a JSON file and filter for future pools."""
    with open(file_path, 'r') as file:
        pools = json.load(file)
    # Filter out pools that have already opened
    return [pool for pool in pools if pool.get("pool_open_time", float('inf')) > current_unix_time()]
...
async def continuously_run():
    while True:
        ...
        if any(task == task_check_pool for task in done):
            ...
            if not pools_data:
                print("No future pools to buy. Waiting for new data.")
                continue  # Skip the buy operations if no future pools are available

            print("Starting the buy operation...")
            start_time = time.time()

            try:
                execute_buy_operation(solana_client, pools_data, config['payer_pubkey_str'], config['amount'])
                end_time = time.time()
                ...
                if not base_mint:
                    ...
                    time.sleep(4)  # Wait 4 sec for base mint
